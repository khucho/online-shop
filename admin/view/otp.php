...
if (isset($_POST['otp_submit']))
{
    if ($_POST['otp'] == $_SESSION['otp'])
    {
        ...
        $status = $auth_controller->addAdmin($_SESSION['data']);

        if (!empty($status))
        {
            session_destroy();
            $registeredId = $auth_controller->adminByEmail($status);
            // die(var_dump($registeredId));
            session_start();
...
<body class="bg-primary">
    ...
    <div class="wrapper">
        <header>Validate Registration</header>
        <form action="" method="post" class="my-4">
            ...
            <div class="d-flex justify-content-center mt-4">
                <input type="submit" value="Sign Up" name="otp_submit" class="btn btn-success w-100">
            </div>
